...
def text2handnotes():
    txt = open("Sample/dummy.txt")  # path of your text file
    # path of page(background)photo (I have used blank page)
    BG = Image.open("Sample/bg.png")
    sheet_width = BG.width
    gap, ht = 0, 0

    for i in txt.read().replace("\n", ""):
        cases = Image.open("Sample/Font1/{}.png".format(str(ord(i))))
...
def draw_bboxes(img, bboxes, color=(0, 0, 255), thickness=1):
    for bbox in bboxes:
        cv2.rectangle(img, tuple(bbox[:2]), tuple(
            bbox[:2]+bbox[-2:]), color, thickness)
...
class Window(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.geometry('450x0+9+60')
        self.title('Capturing Area')
...
def monitor_screen():
    ...
    while (True):
        ...
        if (len(text) > 0):
            print(text + " ; ")
            # TODO: Regex to recognize valid name and address
            '''
            gcodegen = Gcode_Gen()
            gcodegen._load_font("cursive")
            gcodegen._render_stuff(True, captured_text)'''

        time.sleep(0.1)
        # ^\d{1,6}\s(?:[A-Za-z0-9#,]+\s){0,7}(?:[A-Za-z0-9#]+\n)\s*(?:[A-Za-z]+\s){0,3}(?:[A-Za-z]+,)\s*[A-Z]{2}\s*\d{5}$


if (__name__ == "__main__"):
    # text2handnotes()

    # testing text TODO: change this to OCR
    captured_text = """Jacquelyn Fuller
#2323 Country Club Dr
#Pearland, TX 77581"""

    # Area of screen to monitor
    screen_rect = [9, 74, 459 - 9, 319 - 74]
    print("Watching " + str(screen_rect))
    app = App()
    app.open_window()
    app.after(0, monitor_screen)
    app.mainloop()
